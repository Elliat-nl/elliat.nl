...
(:~
 ...
 :)
declare function local:fallback-login (
  $domain as xs:string,
...
else if (starts-with($exist:path, "/CAe")) then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="/newSearch.html?searchType=clavis&amp;clavistype=&amp;query={
        substring-after($exist:path, "/CAe")
      }" />
  </dispatch>
...
(: Resource paths starting with $shared are loaded from the shared-resources app :)
  ...
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <forward
      url="{ $config:appUrl }/shared-resources/{
        substring-after($exist:path, "/$shared/")
      }" />
  </dispatch>
  ...
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <forward
      method="get"
      url="{ $config:appUrl }/openapi/{
        $exist:path => substring-after("/openapi/") => replace("json", "xq")
      }"
    >
      <add-parameter
        name="target"
...
(: Requests for javascript libraries are resolved to the file system :)
  ...
  contains($exist:path, "resources/") and not(contains($exist:path, "api/"))
) then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <forward
      url="{ $exist:controller }/resources/{
        substring-after($exist:path, "resources/")
      }" />
  </dispatch>
(: Requests for javascript libraries are resolved to the file system :)
  ...
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <forward
      url="{ $exist:controller }/vocabularies/{
        substring-after($exist:path, "vocabularies/")
      }" />
  </dispatch>
...
(: Requests for javascript libraries are resolved to the file system :)
  ...
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <forward
      url="{ $exist:controller }/resources/mirador/{
        substring-after($exist:path, "mirador/")
      }" />
  </dispatch>
(: the xql files :)
...
(: special redirect for institutions to redirect to list and bypass standard main view :)
  ...
  return <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    ...
    <error-handler>
      <forward method="get" url="{ $exist:controller }/error.html" />
      <forward url="{ $exist:controller }/modules/view.xql" />
    </error-handler>
  </dispatch>
else if (
  (: :$exist:path = '/as.html' or : :) $exist:path = "/search.html"
(: :or $exist:path = '/facet.html'
...
(: another backward compatibility redirect :)
else if (matches($exist:path, "/manuscripts/INS\d+[\w_-]+/list")) then
  let $insid := replace($exist:path, "/manuscripts/", "")
    => replace("/list", "")
  return <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=mss&amp;reporef={
        $insid
      }" />
  </dispatch>
  ...
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      ...
      url="{
        $config:appUrl
      }/newSearch.html?searchType=placeSearch&amp;mode=any&amp;query={
        request:get-parameter("place", ())
      }" />
  </dispatch>
  ...
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      ...
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=mss" />
  </dispatch>
else if ($exist:path = "/persons/list") then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=pers" />
  </dispatch>
else if ($exist:path = "/institutions/list") then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=ins" />
  </dispatch>
else if ($exist:path = "/ethnic/list") then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=eth" />
  </dispatch>
else if ($exist:path = "/places/list") then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=place&amp;work-types=ins" />
  </dispatch>
else if ($exist:path = "/institutions/list") then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
...
else if ($exist:path = "/narratives/list") then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=nar" />
  </dispatch>
else if ($exist:path = "/works/list") then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=work" />
  </dispatch>
else if ($exist:path = "/studies/list") then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <redirect
      absolute="yes"
      url="{
        $config:appUrl
      }/newSearch.html?searchType=text&amp;mode=any&amp;work-types=studies" />
  </dispatch>
...
(: redirect to api all calls starting with /api/ ending with one of the specific rest modules :)
  ...
  if (ends-with($exist:path, "/")) then
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
      <redirect url="{ $config:appUrl }/apidoc.html" />
    </dispatch>
  else
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
  ...
  contains($exist:path, "Dillmann") and not(contains($exist:path, "PRS"))
) then
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <forward absolute="yes" url="{ concat("/restxq", $exist:path) }">
      { login:set-user($domain, (), false()) }
...
(: redirects to api for geoJson :)
  ...
  <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    <forward
      absolute="yes"
      url="{ $config:appUrl }/restxq/api/geoJson/places/{
        substring-before($exist:resource, ".json")
      }" />
  </dispatch>

(: tei tranformed :)
else if (
  starts-with($exist:path, "/tei/") and ends-with($exist:path, ".xml")
) then
  ...
  return if ($item) then
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
      <forward absolute="yes" url="{ $config:appUrl }/{ $id }.xml" />
    </dispatch>
  else
...
(: the xml stored in the db :)
  ...
  return if ($item) then
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
      <forward url="/{ substring-after($uri, "db/apps/") }" />
      <error-handler>
        <forward
          method="get"
...
(: RDF data as transformed on upload :)
  ...
  return if ($item) then
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
      <forward url="{ $config:appUrl }/rdf/{ $exist:resource }" />
      <error-handler>
        <forward
          method="get"
          url="{ $exist:controller }/error/error-page.html" />
...
function apisparql:constructURIsubid() is called to construct a graph of that resource.
 :)
    ...
    return if (contains(request:get-header("Accept"), "rdf")) then
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward
          absolute="yes"
      ...
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect
          url="{ $config:appUrl }/{ $switchCollection }/{
            $tokenizePath[2]
          }/main#{ $tokenizePath[last()] }" />
      </dispatch>
...
construct the annotation graph if application/rdf+xml is specified
 :)
  else if (matches($exist:path, "/annotation/")) then
    ...
    return if (contains(request:get-header("Accept"), "rdf")) then
      ...
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect url="{ $config:appUrl }/{ $tokenizePath[2] }" />
      </dispatch>

  (: https://betamasaheft.eu/bond/snap:GrandfatherOf-PRS1854Amdase :)

  else if (starts-with($exist:path, "/bond/")) then
    let $tokenizePath := tokenize($exist:path, "/")
    ...
    return if (contains(request:get-header("Accept"), "rdf")) then
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward
          ...
          url="{
            concat(
              "/restxq/BetMasWeb/bond/",
              encode-for-uri($tokenizePath[3]),
              "/rdf"
            )
          }"
        >
      ...
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect url="{ $config:appUrl }/{ $tokenizeBond[2] }" />
      </dispatch>
...
manuscript followed by a passage reference, then go to the text view of that
 ...
 : these will actually go to the api
  :)
    ...
    matches(
      $exist:resource,
      "^(\w+\d+(\w+)?(((_(ED|TR)_([a-zA-Z0-9]+)?)(\.)?)|\.))((NAR[0-9A-Za-z]+|(\d+(r|v)?([a-z])?(\d+)?)|([A-Za-z]+)?([0-9]+))(\.)?)?(\-)?(((NAR[0-9A-Za-z]+|(\d+(r|v)[a-z])|([A-Za-z]+)?([0-9]+))(\.)?)+)?"
    )
  ) then
    let $prefix := substring($exist:resource, 1, 2)
    let $switchCollection := local:switchPrefix($prefix)
    ...
    let $reforrange := if (contains($passage, "-")) then
      "?start=" ||
        substring-before($passage, "-") ||
        "&amp;end=" ||
        substring-after($passage, "-")
    else
      "?ref=" || $passage
  ...
  else if (matches($exist:resource, "^\w+\d+(\w+)?$")) then
    let $prefix := substring($exist:resource, 1, 2)
    let $switchCollection := local:switchPrefix($prefix)
...
https://betamasaheft.eu/manuscript/BNFet32/main
 :) if (contains(request:get-header("Accept"), "rdf")) then
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward
          absolute="yes"
    ...
    else if (contains(request:get-header("Accept"), "tei+xml")) then
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect url="{ concat("/tei/", $exist:resource, ".xml") }" />
      </dispatch>
    else
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect
...
https://betamasaheft.eu/authority-files/angel/main
 :)
  else if (matches($exist:resource, "^[^.\d]+$")) then
    ...
    else (: if it is a special page then go there :) if (
      starts-with($exist:path, "/decorations")
    ) then
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
    ...
    else (: let $c := console:log($exist:resource)
	let $c2 := console:log($taxonomy) return :) (: Inline the canonical loading, it takes a lot of time :)
    if (
      doc("/db/apps/lists/canonicaltaxonomy.xml")//t:category/@xml:id =
        $exist:resource
    ) then
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <redirect
      ...
      <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
        <forward url="{ $exist:controller }/error/error-page.html" />
        <view><forward url="{ $exist:controller }/modules/view.xql" /></view>
      </dispatch>
  else
    <dispatch xmlns="http://exist.sourceforge.net/NS/exist">
